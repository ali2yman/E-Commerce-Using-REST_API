...
@api_view(['POST'])
@permission_classes([IsAuthenticated,IsAdminUser])                 # here only the admin can add a new project
...
def create_review(request,pk):
    ...
    product = get_object_or_404(Product,id=pk)
    data = request.data
    #  This line retrieves any existing review for the product by the current user.
    review = product.reviews.filter(user=user)
   
    if data['rating'] <= 0 or data['rating'] > 10:
        return Response({"error":'Please select between 1 to 9 only'}
                        ,status=status.HTTP_400_BAD_REQUEST) 
    elif review.exists():
        new_review = {'rating':data['rating'], 'comment':data['comment'] }
        review.update(**new_review)

# to get the average for all the existed reviews 
